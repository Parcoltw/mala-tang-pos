...
checkout.onclick=()=>{
  ...
  const rec={
    id:Date.now(),orderNo:no,time:new Date().toISOString(),amount:total(),
    service:pendingOptions.service,spice:pendingOptions.spice,oil:pendingOptions.oil||'',
    bowl:pendingOptions.bowl||'',utensils:pendingOptions.utensils||'',items:cartText()
  };
...
window.renderHistory=()=>{
  ...
  list.innerHTML='';
  if(!records.length){list.innerHTML='<div class="empty">尚無結帳紀錄</div>';return}
